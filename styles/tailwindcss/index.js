module.exports = {
  screens: {
    xl: { min: '1441px' },
    lg: { min: '921px', max: '1440px' },
    md: { min: '769px', max: '920px' },
    sm: { max: '768px' },
    desktop: { min: '769px' },
  },
  colors: {
    base: { main: '#000000', light: '#78909c', lighter: '#f5f5f5', lightest: '#fafafa' },
    primary: { darker: '#2962ff', main: '#2979ff', light: '#bbdefb', lighter: '#e3f2fd' },
    secondary: { darker: '#263238', main: '#37474f', light: '#cfd8dc', lighter: '#eceff1' },
    success: { darker: '#4caf50', main: '#00c853', light: '#c8e6c9', lighter: '#e8f5e9' },
    danger: { darker: '#d50000', main: '#ff5252', light: '#ffcdd2', lighter: '#ffebee' },
    outline: { primary: '#DDEAFF', secondary: '#DFE2E3', success: '#D6F6E3', danger: '#FFE3E3' },
    black: '#000000',
    white: '#ffffff',
    transparent: 'transparent',
    gray: {
      50: '#fafafa',
      100: '#f5f5f5',
      200: '#eeeeee',
      300: '#e0e0e0',
      400: '#bdbdbd',
      500: '#9e9e9e',
      600: '#757575',
      700: '#616161',
      800: '#424242',
      900: '#212121',
    },
    'blue-gray': {
      50: '#eceff1',
      100: '#cfd8dc',
      200: '#b0bec5',
      300: '#90a4ae',
      400: '#78909c',
      500: '#607d8b',
      600: '#546e7a',
      700: '#455a64',
      800: '#37474f',
      900: '#263238',
    },
    blue: {
      50: '#e3f2fd',
      100: '#bbdefb',
      200: '#90caf9',
      300: '#64b5f6',
      400: '#42a5f5',
      500: '#2196f3',
      600: '#1e88e5',
      700: '#1976d2',
      800: '#1565c0',
      900: '#0d47a1',
    },
    'blue-accent': { 100: '#82b1ff', 200: '#448aff', 300: '#2979ff', 400: '#2962ff' },
    green: {
      50: '#e8f5e9',
      100: '#c8e6c9',
      200: '#a5d6a7',
      300: '#81c784',
      400: '#66bb6a',
      500: '#4caf50',
      600: '#43a047',
      700: '#388e3c',
      800: '#2e7d32',
      900: '#1b5e20',
    },
    'green-accent': { 100: '#b9f6ca', 200: '#69f0ae', 300: '#00e676', 400: '#00c853' },
    red: {
      50: '#ffebee',
      100: '#ffcdd2',
      200: '#ef9a9a',
      300: '#e57373',
      400: '#ef5350',
      500: '#f44336',
      600: '#e53935',
      700: '#d32f2f',
      800: '#c62828',
      900: '#b71c1c',
    },
    'red-accent': { 100: '#ff8a80', 200: '#ff5252', 300: '#ff1744', 400: '#d50000' },
    pink: {
      50: '#fce4ec',
      100: '#f8bbd0',
      200: '#f48fb1',
      300: '#f06292',
      400: '#ec407a',
      500: '#e91e63',
      600: '#d81b60',
      700: '#c2185b',
      800: '#ad1457',
      900: '#880e4f',
    },
    'pink-accent': { 100: '#ff80ab', 200: '#ff4081', 300: '#f50057', 400: '#c51162' },
    yellow: {
      50: '#fffde7',
      100: '#fff9c4',
      200: '#fff59d',
      300: '#fff176',
      400: '#ffee58',
      500: '#ffeb3b',
      600: '#fdd835',
      700: '#fbc02d',
      800: '#f9a825',
      900: '#f57f17',
    },
    'yellow-accent': { 100: '#ffff8d', 200: '#ffff00', 300: '#ffea00', 400: '#ffd600' },
    orange: {
      50: '#fff3e0',
      100: '#ffe0b2',
      200: '#ffcc80',
      300: '#ffb74d',
      400: '#ffa726',
      500: '#ff9800',
      600: '#fb8c00',
      700: '#f57c00',
      800: '#ef6c00',
      900: '#e65100',
    },
    'orange-accent': { 100: '#ffd180', 200: '#ffab40', 300: '#ff9100', 400: '#ff6d00' },
  },
  fontSize: {
    h1: ['48px', { lineHeight: '64px', fontWeight: 700 }],
    h2: ['32px', { lineHeight: '40px', fontWeight: 700 }],
    h3: ['24px', { lineHeight: '32px', fontWeight: 700 }],
    h4: ['20px', { lineHeight: '24px', fontWeight: 700 }],
    t1: ['24px', { lineHeight: '36px', fontWeight: 400 }],
    t2: ['20px', { lineHeight: '28px', fontWeight: 400 }],
    t3: ['16px', { lineHeight: '24px', fontWeight: 400 }],
    t4: ['12px', { lineHeight: '16px', fontWeight: 400 }],
    tm1: ['24px', { lineHeight: '36px', fontWeight: 600 }],
    tm2: ['20px', { lineHeight: '28px', fontWeight: 600 }],
    tm3: ['16px', { lineHeight: '24px', fontWeight: 600 }],
    tm4: ['12px', { lineHeight: '16px', fontWeight: 600 }],
  },
  borderRadius: {
    none: '0px',
    xs: '4px',
    sm: '6px',
    md: '8px',
    lg: '12px',
    xl: '16px',
    xxl: '24px',
    full: '9999px',
  },
  extend: {
    boxShadow: {
      xs: '0px 2px 4px -2px rgba(24, 39, 75, 0.12), 0px 4px 4px -2px rgba(24, 39, 75, 0.08)',
      sm: '0px 4px 6px -4px rgba(24, 39, 75, 0.12), 0px 8px 8px -4px rgba(24, 39, 75, 0.08)',
      md: '0px 6px 8px -6px rgba(24, 39, 75, 0.12), 0px 8px 16px -6px rgba(24, 39, 75, 0.08)',
      lg: '0px 6px 12px -6px rgba(24, 39, 75, 0.12), 0px 8px 24px -4px rgba(24, 39, 75, 0.08)',
      xl: '0px 6px 14px -6px rgba(24, 39, 75, 0.12), 0px 10px 32px -4px rgba(24, 39, 75, 0.1)',
      xxl: '0px 8px 18px -6px rgba(24, 39, 75, 0.12), 0px 12px 42px -4px rgba(24, 39, 75, 0.12)',
      xxxl: '0px 8px 22px -6px rgba(24, 39, 75, 0.12), 0px 14px 64px -4px rgba(24, 39, 75, 0.12)',
      xxxxl: '0px 8px 28px -6px rgba(24, 39, 75, 0.12), 0px 18px 88px -4px rgba(24, 39, 75, 0.14)',
    },
  },
};
