...
const getTokensObject = (data) => {
  return Object.keys(data).reduce((acc, cur) => {
    ...
    return {
      ...acc,
      [kebabCase(cur)]: Object.fromEntries(
        Object.values(data[cur]).map((item) => [
          kebabCase(item.path[item.path.length - 1]),
          item.value,
        ]),
      ),
    };
      ...
      return {
        ...
        ...Object.fromEntries(
          Object.keys(variables.font[cur]).map((item) => [
            `${cur}${item}`,
            [
              variables.font[cur][item].size.value,
              {
